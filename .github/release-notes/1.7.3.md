...
**There is no 1.7.2 release.** During the 1.7.2 cut the backend and openscap images were published before the release gate completed and were publicly pullable for roughly nine hours before those tags were deleted; republishing the same number with different content is the "same tag, different bytes" hazard exact-version immutability exists to prevent, so the number was retired rather than re-pointed and everything staged for it ships here. No GitHub Release was ever published under 1.7.2 — you have not missed one — but **if you pulled a `1.7.2` image inside that window, discard it and take 1.7.3**: those bytes carry no release-gate certification, and although the tags no longer resolve in either registry, a local or mirrored cache can still hold a copy.
...
The 1.7.3 schema is forward-compatible with a 1.7.1 binary, but a 1.7.1 binary will **refuse to start** against a 1.7.3 database unless you set `SKIP_MIGRATIONS=true`. You set the flag that sounds dangerous in order to roll back safely; that inversion is exactly what nobody works out mid-incident.
...
**Full Changelog**: https://github.com/artifact-keeper/artifact-keeper/compare/v1.7.1...v1.7.3
